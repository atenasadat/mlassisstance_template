...
from xray_segmentation.Evaluator.xray_evaluator import SegEvaluator
from ..data import XRayLoader


class XRayConfig(NormalConfig):
    ...
    def __init__(self, try_name: str, try_num: int):
        super().__init__(
            ...
            try_name=try_name,
            try_num=try_num,
            evaluator_cls=SegEvaluator,
            content_loaders=[('xray', XRayLoader)],
            inp_size=28
        )
        ...
        self.training_config.augmentations_dict = {
            ...
            #     ,
            'xray_mask': nn.Sequential(
                transforms.Resize((28, 28))
            )
            ,
            'xray_x': nn.Sequential(
                transforms.Resize((28, 28))
            )
